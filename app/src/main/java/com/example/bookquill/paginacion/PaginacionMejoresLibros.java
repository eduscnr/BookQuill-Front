...
public class PaginacionMejoresLibros extends RxPagingSource<Integer, Libro> {
    private String token;

    public PaginacionMejoresLibros() {
    }

    public PaginacionMejoresLibros(String token) {
        this.token = token;
    }
    ...
    public Single<LoadResult<Integer, Libro>> loadSingle(@NonNull LoadParams<Integer> loadParams) {
        ...
        return apiService.getLibrosTop10()
                .subscribeOn(Schedulers.io())
                .map(new Function<List<Libro>, LoadResult<Integer, Libro>>() {
                    @Override
                    public LoadResult<Integer, Libro> apply(List<Libro> libros) throws Throwable {
